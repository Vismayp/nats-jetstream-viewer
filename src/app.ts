...
export function createApp(
  ...
  app.get("/api/profiles", (_req, res) =>
    res.json(store.snapshot().profiles.map(publicProfile)),
  );
  ...
  app.post("/api/connections/reconcile", async (_req, res) => {
    await manager.reconcile(store.snapshot());
    res.json(manager.statuses());
  });
  app.get("/api/profiles/:profileId/streams", async (req, res) =>
    res.json(await manager.listStreams(req.params.profileId)),
  );
  app.get("/api/profiles/:profileId/streams/:stream", async (req, res) =>
    res.json(await manager.streamInfo(req.params.profileId, req.params.stream)),
  );
  app.get(
    "/api/profiles/:profileId/streams/:stream/messages",
    async (req, res) => {
      const query = messageQuery.parse(req.query);
      res.json(
        await manager.messages(req.params.profileId, req.params.stream, query),
      );
    },
  );
  app.get(
    "/api/profiles/:profileId/streams/:stream/messages/:sequence",
    async (req, res) => {
      ...
      const message = await manager.message(
        ...
        req.params.stream,
        sequence,
        req.query.decode !== "false",
      );
      if (!message)
        return res.status(404).json({ error: "Message is no longer stored" });
      return res.json(message);
    },
  );
  ...
  app.post("/api/decoders", async (req, res) => {
    const decoder = await store.upsertDecoder({
      ...req.body,
      id: req.body?.id || randomUUID(),
    });
    await manager.reconcile(store.snapshot());
  ...
  app.post("/api/decoders/test", async (req, res) => {
    ...
    res.json({
      output: await new DecoderSandbox().run(test.script, test.input),
    });
  });

  const dist = resolve(process.env.NJV_WEB_ROOT ?? "./dist");
  app.use(express.static(dist, { index: false, maxAge: "1h" }));
  // Express 5's named wildcard does not match the root path. Braces make the
  // splat optional, so both `/` and client-side routes serve the SPA shell.
  ...
  const errors: ErrorRequestHandler = (error, _req, res, _next) => {
    ...
    const status = /not connected|not found|stream not found/i.test(message)
      ? 503
      : 500;
